    ...
    getPackage: function (req, res) {
        ...
        if(req.query['class_id'] && req.query['courseTypeId']){
                        ...
                        if (errorCourse) {
                           // console.log(errorCourse);return 
                            ...
                            if (idArray.length > 0) {
                                ...
                                package.find({ "courseDetails.courseId": { $in: idArray }, availableForB2C: true, expiryDate: { $gte: today }, tag: {$ne:"ailct"} }, {
           ...
           // console.log("Here");return  
            ...
            package.find({ "courseDetails.courseId": courseId, availableForB2C: true, expiryDate: { $gte: today }, tag:{ $ne:"ailct"}}, {
                expiryDate: 1, targetYear: 1, name: 1, sku: 1, description: 1, price: 1, discount: 1, courseDetails: 1
    ...
    getAllPackage: function (req, res) {
        ...
        course.aggregate([{ $unwind: "$eligibility" }, { $match: { "eligibility.sessionStartTime": {$in:sessionStartYear} } }, { $project: { _id: 1 } }]).exec(function (errorCourse, dataCourse) {
            ...
            if (errorCourse) {
                ...
                if (idArray.length > 0) {
                    var today = new Date();
                    package.find({ "courseDetails.courseId": { $in: idArray }, availableForB2C: true, expiryDate: { $gte: today }, tag: {$ne:"ailct"} }).sort({ price: 1 }).populate('courseDetails.courseId')
