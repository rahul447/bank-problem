...
function courseItemConditionCheck(query, courseItemCallback) {
    CourseItems.findOne(query, function (err, courseItemResult) {
        if (err) {
            console.log(err);
            return courseItemCallback(err);
        }
        else {
            if (courseItemResult) {
                console.log('in');
                var testFoundInCourse = 1;
                return courseItemCallback(null, testFoundInCourse);
            }
            else {
                return courseItemCallback();
            }
        }
    })
}
    ...
    */
    testpaper: function (req, res) {
        try {
            ...
            var testFoundInCourse = 0;
            var courseName = "";
            console.log(req.query);
            courseModel.findOne({ '_id': courseId }, { 'courseItems': 1, name: 1 }, function (err, courseItemArray) {
                if (err) {
                    console.log(err);
                    console.log('error in courseModel');
                    return res.json({
                        'status': '500',
                    ...
                    if (!courseItemArray)
                        ...
                        courseName = courseItemArray.name;
                        async.eachLimit(courseItemArray.courseItems, 5, function (courseItem, courseItemCallback) {
                            console.log(courseItem);
                            var query = {
                                '_id': courseItem.id,
                              };
                            if (courseItem.itemType === 'scheduledTests') {
                                query['details.schedule.paperDetail.id'] = testId;
                            }
                            else if (courseItem.itemType === 'sampleTests') {
                                query['details.sampletests.paperDetail.id'] = testId;
                            }
                            else if (courseItem.itemType === 'testGroup') {
                                query['details.tests.paperDetail.id'] = testId;
                            }
                            else if (courseItem.itemType === 'orgTests') {
                                query['details.orgTests.paperDetail.id'] = testId;
                            }
                            else {
                                return courseItemCallback();
                            }   
                            courseItemConditionCheck(query, (err, data) => {
                                testFoundInCourse = data ? 1 : testFoundInCourse;
                                return courseItemCallback(err, data)
                            });
                        }, function (err) {
                            if (err) {
                                console.log(err);
                                console.log('error in courseItem')
                                return res.json({
                                    'status': '500',
                                    'message': 'Error in retrieving test'
                                });
                                ...
                                if (testFoundInCourse) {
                                    console.log('sending test');
                                    test.findOne({ '_id': testId, 'data.sections': { $exists: true } }).populate('data.sections.subSection.questions.qId').exec(function (err, test) {
                                    ...
                                    return res.json({
                                        'status': '500',
                                        'message': 'No test Found In CourseItem'
                                    });
                                }
                            }
                        }
    ...
    getAllTestByPagination: function (req, res) {
        ...
        if (req.query.uniqueId)
            queryObjName['uploadDetail.uniqueId'] = new RegExp(req.query.uniqueId, 'i');
        ...
        if (req.query.uniqueId)
            queryObjSql['uploadDetail.uniqueId'] = new RegExp(req.query.uniqueId, 'i');
        ...
        // if(req.query.testNameInput)
        //     queryObjMigSql['tags.usedInCourse.sqlId'] = req.query.testNameInput;
        //, queryObjMigSql
    ...
    getAllTestCount: function (req, res) {
        ...
        if (req.query.uniqueId)
            queryObjName['uploadDetail.uniqueId'] = req.query.uniqueId;
