        ...
        if(err1){
          ...
          // console.log(typeof classId);return 
           ...
           if (month < 4) {
            ...
            customObj['originalClass']=classType['name'];
            ...
            if(err3){
                ...
                .exec(function (err, courses) {
                    ...
                    if (err) {
                        ...
                        if(data3['originalClass']=="12th +"){
                            var courses=courses.filter(function(item){
                                var str=item.name.toLowerCase();
                            
                                var name=str.match(/kvpy/g);
                            ...
                            if (tests.courseDetails[j].includedItems[k].filter == 'all') {
                                for (var ptr = 0; ptr < tests.courseDetails[j].includedItems[k].itemId.details.schedule.length; ptr++) {
                                    if (tests.courseDetails[j].includedItems[k].itemId.details.schedule[ptr].paperDetail.type == 'full')
                                        full.push(tests.courseDetails[j].includedItems[k].itemId.details.schedule[ptr].scheduleID.toString());
                                    else if (tests.courseDetails[j].includedItems[k].itemId.details.schedule[ptr].paperDetail.type == 'part')
                                        part.push(tests.courseDetails[j].includedItems[k].itemId.details.schedule[ptr].scheduleID.toString());
                                    else{
                                        combined.push(tests.courseDetails[j].includedItems[k].itemId.details.schedule[ptr].scheduleID.toString());
