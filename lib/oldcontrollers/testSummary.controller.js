...
const updateTestName = () => {
    test.find({}, { name: 1, settings: 1 }, (err, testData) => {
        async.eachLimit(testData, 50, (test, testCallback) => {
            TestSummary.update({ testId: test._id }, { $set: { testName: test.name, testType: test.settings.testType } }, { multi: true }, (err, data) => {
                if (err)
                    return testCallback(err);
                console.log('this test is updated', test.name);
                return testCallback();
            })
        }, (err) => {
...
module.exports = {
    ...
    */
    forceEntry: function (req, res) {
        Course.find({ $or: [{ name: /.*NTSE*./ }, { name: /.*KVPY*./ }] }, { courseItems: 1, name: 1 }, function (err, courseIdArray) {
            if (err) {
                res.send(err);
            }
            else {
                ...
                async.eachLimit(courseIdArray, 1, function (course, courseCallback) {
                    var courseId = course._id;
                    // console.log('courseId -> ',courseId);
                    var courseItemArray = course.courseItems;
                    var courseName = course.name;
                    async.eachLimit(courseItemArray, 1, function (courseItemCourseObj, courseItemCourseObjCallback) {
                        ...
                        CourseItems.find({ _id: courseItemId }, { details: 1 }, function (err, courseItems) {
                            if (err) {
                                ...
                                async.eachLimit(courseItems, 1, function (courseItem, courseItemCallback) {
                                    var details = courseItem.details;
                                    // console.log('details -> ',details);
                                    if ('tests' in details && details.tests.length)
                                        checkPaper(details.tests, courseId, courseName, function (err) {
                                            if (err) {
                                                courseItemCallback(err);
                                            }
                                            else {
                                        ...
                                        checkPaper(details.schedule, courseId, courseName, function (err) {
                                            if (err) {
                                                courseItemCallback(err);
                                            }
                                            else {
                                        ...
                                        checkPaper(details.sampletests, courseId, courseName, function (err) {
                                            if (err) {
                                                courseItemCallback(err);
                                            }
                                            else {
                    ...
                    if (err) {
                        ...
                        async.eachSeries(testId_courseId, function (element, callback) {
                            Question.findOne({ 'conceptId': element.conceptId }, { conceptCode: 1 }, function (err, result) {
                                if (result.conceptCode)
                                    element.conceptCode = result.conceptCode;
                                saveExcel.push(element);
                                console.log(element);
                                console.log('******************');
                                callback();
                            });
                            ...
                            if (err) {
                                ...
                                var writer = csvWriter()
                                writer.pipe(fs.createWriteStream('new.csv'))
                                for (var i = 0; i < saveExcel.length; i++) {
                                    var obj = saveExcel[i];
                                    writer.write(obj);
                                    if (i == saveExcel.length - 1)
                                        writer.end();
                                }
                            }
                        }
    ...
    createTestSummary: function (req, res) {
        ...
        let calculatedTestSummary = Object.assign({}, { status: '', aclMetaData: {}, subSubjects: [], subjects: [], chapters: [], concepts: [] }, req.body);
        
        !calculatedTestSummary.testType ? calculatedTestSummary.testType = '': '';
        !calculatedTestSummary.testName ? calculatedTestSummary.testName = '': ''; 
        !calculatedTestSummary.isOrgTest ? calculatedTestSummary.isOrgTest = false: calculatedTestSummary.isOrgTest;
        async.parallel([
            function (firstcb) {
                test.findOne({ '_id': testId }, function (err, data) {
                    if (err) {
                        ...
                        calculatedTestSummary.aclMetaData = data.aclMetaData ? data.aclMetaData : {};
                        calculatedTestSummary.status = data.status;
                        calculatedTestSummary.testName = data.name;
                        calculatedTestSummary.testType = data.settings.testType;
                        var sections = data.data.sections;
                        //console.log('test data received!', data);
                        var qIds = [];
                        //console.log(data.sections);
                        async.eachLimit(sections, 1, function (section, sectioncallback) {
                            //console.log("in section");
                            // console.log(section.subSection.length);
                            async.eachLimit(section.subSection, 1, function (subSection, subSectionCallback) {
                            ...
                            if (err) {
                                ...
                                var map = [];
                                //console.log(qIds);
                                async.eachLimit(qIds, 1, function (qid, qcb) {
            ...
            if (err) {
                ...
                var flag = 0;
                async.eachLimit(conceptIds, 1, function (conceptId, conceptIdCallback) {
                    ...
                    if (index1 > -1) {
                        ...
                        var objj = {
                            ...
                            'sqlTestId': req.body.sqlId,
                            'questionId': cqId[conceptId],
                            "message": "Not FOund in CourseSyllabus"
                        };
                        testId_courseId.push(objj);
                        conceptIdCallback();
                    }
                    ...
                    if (err) {
                        ...
                        if (dataToBeSend.length) {
                            return res.json({
                                'status': '200',
                                'message': 'this data not found in courseSyllabus',
                                'data': dataToBeSend
                            });
                        }
                        else {
                            ...
                            for (let i = 0; i < calculatedTestSummary.chapters.length; i++) {
                                let concepts = calculatedTestSummary.chapters[i].concepts;
                                calculatedTestSummary.chapters[i].concepts = [];
                                for (let j = 0; j < calculatedTestSummary.concepts.length; j++) {
                                    for (let k = 0; k < concepts.length; k++) {
                                        if (concepts[k].toString() === calculatedTestSummary.concepts[j].id.toString()) {
                                            calculatedTestSummary.chapters[i].concepts.push(concepts[k])
                                        }
                                    }
                                }
                            }
    ...
    createOrgTestSummary: function (testId, courseId) {
        ...
        apiCall(url + '/createTestSummary', data, (err, data) => {
            if (err) {
                console.log(err);
            }
            else {
