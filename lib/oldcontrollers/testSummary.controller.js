...
function checkPaper(arrayObj, courseId,courseName,incremental, cb) {
       ...
       // console.log(arrayel); 
        if(arrayel.paperDetail['id']== "undefined"){
           ...
           // console.log(testId);
            ...
            TestSummary.findOne({testId:testId},{_id:1},(err,data)=>{
                if(err)
                    return testCallback(err);
                else if(data) {
                    console.log('test summary data ',data,incremental);
                    if(!incremental)
...
module.exports = {
    ...
    */
    forceEntry: function (req, res) {
        var incremental = req.body.incrementalFlag?req.body.incrementalFlag:false;
        Course.find({}, { courseItems: 1, name: 1 }, function (err, courseIdArray) {
            if (err) {
                res.send(err);
            }
            else {
                            ...
                            if (err) {
                                ...
                                async.eachLimit(courseItems, 1, function (courseItem, courseItemCallback) {
                                    ...
                                    if ('tests' in details && details.tests.length)
                                        checkPaper(details.tests, courseId, courseName,incremental, function (err) {
                                            if (err) {
                                                courseItemCallback(err);
                                            }
                                            else {
                                        ...
                                        checkPaper(details.schedule, courseId, courseName,incremental, function (err) {
                                            if (err) {
                                                courseItemCallback(err);
                                            }
                                            else {
                                        ...
                                        checkPaper(details.sampletests, courseId, courseName,incremental, function (err) {
                                            if (err) {
                                                courseItemCallback(err);
                                            }
                                            else {
