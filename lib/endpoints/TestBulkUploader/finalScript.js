...
function mamothWork(filepath, body, res) {
    mammoth.convertToHtml({path: filepath}, validationOptions)
        .then(function (result) {
            async.waterfall([
                ...
                if (err) {
                    res.status(200).jsonp({data: err, statusCode: 401 });
                    //saveStatus(i, err);
                }
                else {
                    var errA = [];
                    ...
                    //saveStatus(i, errA);
                    res.status(200).jsonp({data: errA, statusCode: 200 });
                }
            })
        })
}
...
exports.createPaper = async function (req, res) {
    ...
    mamothWork(filepath, req.body, res);
};
