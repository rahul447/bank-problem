var fs = require('fs');
var mammoth = require("lib/endpoints/TestBulkUploader/mammoth");
                                ...
                                async.eachSeries(tempSubSection, function (subSection, subsectionback) {
                                    ...
                                    async.eachSeries(tempQuestion,
                                        function (element, questionback) {
                                            ...
                                            //console.log(data[0]);
                                            if (data[0].replace(/<[^>]*>/g, '').trim() == "MCQ") {
                                                console.log("mcq");
                                                mcq(data, conceptArray,conceptsObjArray, function (returnObj, sNo) {
...
var mcq = function (mcqobj, conceptArray,conceptsObjArray, dbsave) {
    var mcqdbobj = {
        questionType: "MCQ",
        questionCode:0,
        level: "",
...
var passage = function (passageobj, conceptArray,conceptsObjArray, dbsave) {
    ...
    for (var i = 1; i < subQuestionArray.length; i++) {
        ...
        if (subQuestion[0].replace(/<[^>]*>/g, '').trim() == "MCQ") {
