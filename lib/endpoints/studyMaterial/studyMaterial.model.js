...
StudyMaterialSchema.pre('save', async function(next) {
    ...
    if(!exists) {
        console.log("\n********************************************");
        console.log("pre hook for contentId: \n");
...
StudyMaterialSchema.methods.checkValidity = function() {
    ...
    const schemaFields = Array.from(Object.keys(StudyMaterialSchema.paths)
        .reduce((acc, curr) => {
...
StudyMaterialSchema.pre('save', async function(next) {
    if(this.status === "PUBLISHED" && await this.findStatusById(this._id, this.status) !== this.status) {
        console.log("\n********************************************");
        console.log("pre hook for publish: \n");
        this.checkValidity()
        .then((status) => {
            if(status.length === 0) {
                next();
            } else {
                next(new Error(`material has values for (${status}) missing `));
            }
        }).catch(err => {
