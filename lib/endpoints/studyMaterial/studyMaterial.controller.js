...
let StudyMaterialControllerInstance,
...
class StudyMaterialController {
    ...
    updateStudyMaterial(req, res){
        ...
        ((that) => {
            ...
            .then(function(newObj) {
                that.modelInstance.findOneAndUpdate({ _id: id }, newObj, { new: true, populate: 'draftId' })
                .then(function (newObj) {
    ...
    publishMaterial(materialId, aclMetaData, publish) {
        return new Promise((resolve, reject) => {
            ((that) => {
                    ...
                    if(err) {
                        ...
                        if(doc.draftId) {
                            ...
                            if(invalid.length !== 0) {
                                ...
                                that.deleteMaterial(doc.draftId)
                                    .then(() => {
                                        ...
                                        that.modelInstance.findOneAndUpdate({_id: doc._id}, doc,
                                            {upsert: true},
                                            function (err) {
                                                if(err) {
                                                    that.loggerInstance.error(`Material Save Failed 
                                                    ${err}`);
                                                    reject(err);
                                                }
                                                resolve();
                            ...
                            if(invalid.length !== 0) {
                                ...
                                that.modelInstance.findOneAndUpdate({_id: doc._id}, doc, {upsert: true},
                                    function (err) {
                                        if(err) {
    ...
    deleteMaterial(materialId) {
        return new Promise((resolve, reject) => {
            try{
                this.modelInstance.find({ _id: materialId }).remove()
                    .exec();
                resolve();
            } catch(err) {
                this.loggerInstance.error(`DB error removing material ${err}`);
                reject(err);
            }
        });
    }
    ...
    updateMaterialConcept(req, res) {
        ...
        .then(newObj => {
            ...
            this.modelInstance.findOneAndUpdate(conditions, newObj, { new: true, populate: 'draftId' })
            .then(materialObj => {
    ...
    validateDoc(material) {

        let status = {'conceptId': true, 'title': true, 'content': true, 'concepts': true};

        const schemaFields = Array.from(Object.keys(material)
            .reduce((acc, curr) => {
        ...
        function validate(key) {
            ...
            if(key === 'subjects' && (!material["subjects"] || material["subjects"].length === 0)) {
...
export function getStudyMaterialControllerInstance() {
    StudyMaterialControllerInstance = StudyMaterialControllerInstance || new StudyMaterialController(loggerInstance, config);
    return StudyMaterialControllerInstance;
}