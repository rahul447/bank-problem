...
export class StudyMaterialController {
    ...
    createMaterialClone(oldObj, bodyObj) {
        return new Promise((resolve, reject) => {
            ...
            let material = new this.modelInstance(newClonedObj);
            ((that) => {
                material.save()
            ...
            ((that) => {
                that.modelInstance.findById(materialId, function (err, doc) {
                    if(err) {
                        ...
                        if(doc.publishId) {
                            that.deleteMaterial(doc.publishId)
                            .then(() => {
                                that.loggerInstance.info("Material Soft Delete Success");
                            ...
                            .catch(err => {
                                that.loggerInstance.error(`Soft delete Material Error ${err}`);
                                return res.json(new ResponseController(500, `Soft delete Material Error`,
                                err));
                             })
                        } else {
                            doc.status = "PUBLISHED";
