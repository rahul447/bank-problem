...
class StudyMaterialController {
    ...
    publishMaterial(materialId, aclMetaData) {
        return new Promise((resolve, reject) => {
            ((that) => {
    ...
    validateMaterial(Id) {
        return new Promise((resolve, reject) => {
            ...
            exec((err, doc) => {
                if(err) {
                    this.loggerInstance.error(`DB error getting material ${err}`);
                    reject(err);
                } else {
                    this.loggerInstance.info(`Validating material`);
