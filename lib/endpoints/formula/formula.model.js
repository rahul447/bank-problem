...
FormulaSchema.pre('save', async function(next) {
    ...
    if(!exists && !this.publishId) {
        const lastContentId = await this.getMaxContentId();
        this.contentId = !lastContentId ? 'F1': lastContentId.slice(0, 1) + (parseInt(lastContentId.slice(1)) + 1);
        next();
    } else
        next();
});
...
FormulaSchema.methods.checkValidity = function() {
    ...
    let formula = this;
    const schemaFields = Object.keys(FormulaSchema.paths);
