...
export class FormulaController {
    ...
    publishFormula(req, res, formulaId) {
        return new Promise((resolve, reject) => {
            ((that) => {
                that.modelInstance.findById(formulaId, function (err, doc) {
                    if(err) {
                        ...
                        if(doc.publishId) {
                            that.deleteFormula(doc.publishId)
                            .then(() => {
                                that.loggerInstance.info("Formula Soft Delete Success");
                            ...
                            .catch(err => {
                                that.loggerInstance.error(`Soft delete Formula Error ${err}`);
                                return res.json(new ResponseController(500, `Soft delete Formula Error`,
                                    err));
                            })
                        } else {
                            doc.status = "PUBLISHED";
