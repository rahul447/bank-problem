...
class FormulaController {
    ...
    publishFormula(formulaId, aclMetaData, publish) {
        return new Promise((resolve, reject) => {
            ((that) => {
                    ...
                    if(err) {
                        ...
                        if(doc.draftId) {
                            ...
                            if(invalid.length !== 0) {
                                ...
                                that.deleteFormula(doc.draftId)
                                    .then(() => {
                                        ...
                                        that.modelInstance.findOneAndUpdate({_id: doc._id}, doc,
                                            {upsert: true}, function (err) {
                                                if(err) {
                                                    that.loggerInstance.error(`formula Save Failed 
                                                    ${err}`);
                                                    reject(err);
                                                }
                                                resolve();
    ...
    deleteFormula(formulaId) {
        return new Promise((resolve, reject) => {
            try{
                this.modelInstance.find({ _id: formulaId }).remove().exec();
                resolve();
            } catch(err) {
                this.loggerInstance.error(`DB error removing formula ${err}`);
                reject(err);
            }
        });
    }
    ...
    validateDoc(formula) {
        ...
        const schemaFields = Array.from(Object.keys(formula)
            .reduce((acc, curr) => {
