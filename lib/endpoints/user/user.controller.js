...
import {getRoleControllerInstance} from "../role/role.controller";
...
class UserController {
    ...
    constructor(loggerInstance, config, roleInstance) {
    ...
    login(req, res) {
        ...
        ((that) => {
            that.userModel.findOne({ email: email }).
            populate('role').
            populate('clientId').
                   ...
                   that.userModel.findOne({ email: newEmail }, function (err, user) {
                       ...
                       if (user) {
                           ...
                           let newUser = new that.userModel({
                               ...
                               centers: req.body.centers && JSON.parse(req.body.centers),
                               centerId: req.body.centerId && req.body.centerId,
                               defaultShare: req.body.defaultShare && JSON.parse(req.body.defaultShare)
                           });
    ...
    pushTokenToDb(req, res) {
        console.log(" req body", req.body);

        const userId = req.body._id;

        this.userModel.findOne({ _id: mongoose.Types.ObjectId(userId) }, (err, doc) => {
            if (err) {
                this.loggerInstance.error("DB Error");
                return res.json(new ResponseController(err.statusCode, "DB Error", err));
            } else {
                doc.token = req.body.authToken;
                doc.save(() => {
                    this.loggerInstance.info("Token Store Success");
                    return res.json(new ResponseController(200, "Token Store Success"));
                });
            }
        });
    }
    ...
    logout(req, res) {
        this.userModel.findOne({_id: mongoose.Types.ObjectId(req.body.id)}, (err, user) => {
...
export function getUserControllerInstance() {
    UserControllerInstance = UserControllerInstance || new UserController(loggerInstance, config, getRoleControllerInstance());
