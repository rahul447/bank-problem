...
class ClientController {
    ...
    addCenter(orgCenterData) {
        return new Promise((resolve, reject) => {
            this.modelInstance.findOne({ _id: orgCenterData.clientId }, (err, doc) => {
                doc.orgCenters ? doc.orgCenters.push({id: orgCenterData._id, name: orgCenterData.name}):
                    '';
                this.loggerInstance.info(`Center added to client Success`);
                doc.save((err) => {
                    err ? (this.loggerInstance.error(`Center added to client Failure ${err}`), reject(err)
                    ): resolve();
                });
            });
        });
...
export function getClientControllerInstance() {
    ClientControllerInstance = ClientControllerInstance || new ClientController(loggerInstance, config);
