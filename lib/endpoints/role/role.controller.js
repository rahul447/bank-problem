...
class RoleController {
    ...
    getOrgRoleDetailsFromName(req, res) {
        const {
            roleName, clientId
        } = req.query;
        if (!roleName) {
            return sendClientError({
                res,
                responseMessage: 'roleName is missing'
            });
        }
        if (!clientId) {
            return sendClientError({
                res,
                responseMessage: 'clientId is missing'
            });
        }
        this.roleModel.findOne({
            ...
            if (!roleDetails) {
                return sendClientError({
                    res,
                    responseMessage: 'Not found appropriate role',
                    statusCode: 404
                });
            }
            this.loggerInstance.info("Role Get Success");
...
export function getRoleControllerInstance() {
    RoleControllerInstance = RoleControllerInstance || new RoleController(loggerInstance, config);
    return RoleControllerInstance;
}