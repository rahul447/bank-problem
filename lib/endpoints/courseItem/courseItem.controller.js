...
let courseItemControllerInstance,
...
class courseItemController {
    ...
    async createNewCourseItem(req, res) {
        try{
    ...
    async createItemWithTest({ itemName, itemType, courseId, clientId, userId, aclMetaData, test }) {
        ...
        return new Promise(async (resolve, reject) => {
            try{
                        ...
                        orgTests: [
                            {
                                ...
                                testType: test.settings.testType,
                                duration: test.settings.duration,
                                status: test.status,
                                codeName: test.testCode,
                ...
                if(!masterCourse.courseItems.find(item => item.itemType === itemType)) {
                    masterCourse.courseItems.push({
                        id: courseitem._id,
                        itemName,
                        itemType
                    });
                    masterCourse.aclMetaData.updatedBy = aclMetaData.updatedBy;
    ...
    async addOrgTestToItem ({ courseItemId, test }) {
        const courseItem = await this.modelInstance.findById(courseItemId);
        const newTest = {
            paperDetail: {
                syllabus: test.syllabus,
                id: test._id,
        ...
        let existingTestIndex = courseItem.details.orgTests.findIndex(t => t.paperDetail.id.equals(test._id));
        existingTestIndex >= 0 ? courseItem.details.orgTests[existingTestIndex] = newTest : courseItem.details.orgTests.push(newTest);
...
export function getcourseItemControllerInstance() {
    courseItemControllerInstance = courseItemControllerInstance || new courseItemController(loggerInstance, config);
    return courseItemControllerInstance;
}