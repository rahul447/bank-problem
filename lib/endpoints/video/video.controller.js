...
let VideoControllerInstance, {NODE_ENV} = process.env,
...
class VideoController {
    ...
    saveNewVideo(reqBody, reqQuery, res) {
        ...
        VideoObj.aclMetaData = reqBody.aclMetaData;
    ...
    editVideo(req, res){
        ...
        ((that) => {
            ...
            .then(function(newObj) {
                ...
                that.videosModelInstance.findOneAndUpdate(conditions,newObj,{new:true, populate: 'draftId'})
    ...
    publishVideo(videoId, aclMetaData, publish = true) {
        return new Promise((resolve, reject) => {
            ((that) => {
                that.videosModelInstance.findOne({ _id: videoId }).
                populate('draftId').
                        ...
                        if(doc.draftId) {
                            ...
                            if(invalid.length !== 0) {
                                ...
                                .then(() => {
                                    ...
                                    that.videosModelInstance.findOneAndUpdate({_id: doc._id}, doc,
                            ...
                            if(invalid.length !== 0) {
                                ...
                                that.videosModelInstance.findOneAndUpdate({_id: doc._id}, doc, {upsert:
                                            true}, function (err) {
                                        if(err) {
                                            that.loggerInstance.error(`Video Save Failed ${err}`);
                                            reject(err);
    ...
    deleteVideo(videoId) {
        return new Promise((resolve, reject) => {
            try{
                this.videosModelInstance.find({ _id: videoId }).remove()
                    .exec();
                resolve();
            } catch(err) {
                this.loggerInstance.error(`DB error removing video ${err}`);
                reject(err);
            }
        });
    }
    ...
    updateVideoConcept(req, res) {
        ...
        .then(newObj => {
            let conditions = { _id: mongoose.Types.ObjectId(id) };
            this.videosModelInstance.findOneAndUpdate(conditions, newObj, { new: true, populate: 'draftId' })
            .then(videoObj => {
    ...
    validateDoc(video) {
        ...
        let status = {'conceptId': true, 'title': true, 'videos': true, 'concepts': true};
        const schemaFields = Array.from(Object.keys(video)
            .reduce((acc, curr) => {
        ...
        function validate(key) {
            if(key === 'subjects' && (!video["subjects"] || video["subjects"].length === 0)) {
...
export function getVideoControllerInstance() {
    VideoControllerInstance = VideoControllerInstance || new VideoController(loggerInstance, config);
