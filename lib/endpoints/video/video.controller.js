...
export class VideoController {
    ...
    editVideo(req, res){
        ...
        ((that) => {
            that.videosModelInstance.findOne({ _id: mongoose.Types.ObjectId(video._id)})
            .then(function (newObj) {
                return new Promise((resolve, reject) => {
                    if(newObj.status === "PUBLISHED") {
                        that.createVideoClone(newObj, reqBody)
            ...
            ((that) => {
                that.videosModelInstance.findById(videoId, function (err, doc) {
                    if(err) {
                        ...
                        if(doc.publishId) {
                            that.deleteVideo(doc.publishId)
                                .then(() => {
                                    that.loggerInstance.info("Video Soft Delete Success");
                                ...
                                .catch(err => {
                                    that.loggerInstance.error(`Soft delete Video Error ${err}`);
                                    return res.json(new ResponseController(500, `Soft delete Video Error`,
                                        err));
                                })
                        } else {
                            doc.status = "PUBLISHED";
