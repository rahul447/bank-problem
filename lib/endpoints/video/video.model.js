...
VideoSchema.methods.checkValidity = function() {
    ...
    let video = this;
    const schemaFields = Array.from(Object.keys(VideoSchema.paths)
        .reduce((acc, curr) => {
...
VideoSchema.pre('save', async function(next) {
    ...
    if(!exists) {
        ...
        if(this.publishId) {
            ...
            this.contentId = !lastContentId ? 'V-1': `V-${lastContentId + 1}`;
...
VideoSchema.methods.getMaxContentId = () => {
    return new Promise((resolve, reject) => {
        mongoose.models['Video'].find({}, {docCounter: 1}).sort({docCounter:-1}).limit(1)
        .exec((err, doc) => {
...
VideoSchema.pre('save', async function(next) {
    if(this.status === "PUBLISHED" && (await this.findStatusById(this._id, this.status) !== this.status || this.fromDraft)) {
        this.checkValidity()
            .then((invalid) => {
                if(invalid.length === 0) {
                    next();
