...
export class AudioController {
    ...
    editAudio(req, res){
        ((that) => {
            that.audiosModelInstance.findOne({ _id: mongoose.Types.ObjectId(req.query.id)}).lean()
    ...
    updateAudioTags(req, res) {
        ((that) => {
            that.audiosModelInstance.findOne({ _id: mongoose.Types.ObjectId(req.params.id)})
            .then(function (newObj) {
                return new Promise((resolve, reject) => {
                    if(newObj.status === "PUBLISHED" && !newObj.draftId) {
                        that.createAudioClone(newObj, req.body)
