...
export class AudioController {
    ...
    editAudio(req, res){
        ...
        ((that) => {
            that.audiosModelInstance.findOne({ _id: mongoose.Types.ObjectId(audio._id)})
            .then(function (newObj) {
                return new Promise((resolve, reject) => {
                    if(newObj.status === "PUBLISHED") {
                        that.createAudioClone(newObj, reqBody)
                        .then((cloneAudio) => {
            ...
            ((that) => {
                that.audiosModelInstance.findById(audioId, function (err, doc) {
                    if(err) {
                        ...
                        if(doc.publishId) {
                            that.deleteAudio(doc.publishId)
                            .then(() => {
                                that.loggerInstance.info("Audio Soft Delete Success");
                                doc.status = "PUBLISHED";
                                Reflect.deleteProperty(doc, "publishId");
                                doc.save(function (err) {
                                    if(err) {
                            ...
                            .catch(err => {
                                that.loggerInstance.error(`Soft delete Audio Error ${err}`);
                                return res.json(new ResponseController(500, `Soft delete Audio Error`,
                                    err));
                            })
                        } else {
                            doc.status = "PUBLISHED";
