...
export class AudioController {
    ...
    saveNewAudio(reqBody, reqQuery, res) {
        ...
        AudioObj.clientId = reqBody.clientId;
        AudioObj.userId = reqBody.userId;
        AudioObj.lastUpdateBy = reqBody.lastUpdateBy;
    ...
    updateNewAudio(reqBody, reqQuery, res) {
        ...
        ((that) => {
            ...
            let conditions = { _id: mongoose.Types.ObjectId(reqQuery.id) }
                , update = { $push: { content: { $each: reqBody.content } }, $set: { lastUpdateBy:
                    reqBody.lastUpdateBy } }
                , options = { multi: false };
    ...
    publishAudio(audioId, lastUpdateBy) {
        return new Promise((resolve, reject) => {
            ((that) => {
                that.audiosModelInstance.findOne({ _id: audioId }).
                populate('draftId').
                exec(function (err, doc) {
                    if(err) {
                        ...
                        if(doc.draftId) {
                            ...
                            doc.lastUpdateBy = lastUpdateBy;
                            that.deleteAudio(doc.draftId._id, lastUpdateBy)
                                .then(() => {
    ...
    deleteAudio(audioId, lastUpdateBy) {
        return new Promise((resolve, reject) => {
            ((that) => {
    ...
    validateAudio(audioId) {
        return new Promise((resolve, reject) => {
            this.audiosModelInstance.
            findOne({ _id: audioId }).
            populate('draftId').
            exec((err, doc) => {
                if(err) {
                    this.loggerInstance.error(`DB error getting audio ${err}`);
                    reject(err);
                } else {
                    this.loggerInstance.info(`Validating audio`);
