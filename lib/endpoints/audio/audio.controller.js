...
export class AudioController {
    ...
    updateAudioConcept(req, res) {
        ...
        this.audiosModelInstance.findOne({ _id: mongoose.Types.ObjectId(req.params.id) }, (err, doc) => {
            doc.subjects = req.body.subjects;
            doc.save((err, data) => {
                if(err) {
                    this.loggerInstance.error(`Concept Changed for Audio ${req.params.id}`);
                    return res.json({
                        status: 500,
                        data: err,
                        message: `Concept add Error for audio ${req.params.id}`
                    })
                }
                ...
                return res.json({
                    status: 200,
                    data: data,
                    message: `Concept add Success for audio ${req.params.id}`
                })
            });
        });
    }
