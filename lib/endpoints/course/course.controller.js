...
export class CourseController {
    ...
    */
    ...
    addTestsToCourses({courses, aclMetaData }) {
        ...
        return new Promise((resolve, reject) => {
            this.modelInstance.find({ _id: { "$in": courseIds } }, 'courseItems', (err, docs) => {
                if(err) reject(err);
                else {
                    docs.map(async doc => {
                        if(!doc.courseItems.find(item => item.itemType === 'orgTests')) {
                            let item = 'orgTests';
                            await this.courseItemController.create({itemName: item, itemType: item,
                                courseId: doc._id, clientId: aclMetaData.clientId,
                                userId: aclMetaData.userId, aclMetaData: aclMetaData });
                        }
                    });
                    resolve();
                }
