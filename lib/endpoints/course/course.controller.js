...
export class CourseController {
    ...
    getCourse(req, res) {
        if (req.params.id){
            this.getCourseById.call(this, req.params.id, res);
        } else {
            let query = {};
    ...
    getCourseById(id, res) {
        this.modelInstance.findById(id)
            .then(course => {
                ...
                this.loggerInstance.info("Retrieved course list");
                res.json(new ResponseController(200, "Course list retrieved successfully", course));
            })
            .catch(err => {
                this.loggerInstance.debug("DB error getting course");
                res.json(new ResponseController(500, "Error getting course"));
            });
    }
