...
import {getcourseItemControllerInstance} from "../courseItem/courseItem.controller";
import {
    sendClientError,
    sendServerError,
    sendSuccess
...
const oldTestSummaryController = require('../../oldcontrollers/testSummary.controller');

let CourseControllerInstance,
...
export class CourseController {
    ...
    constructor(loggerInstance, config, courseItemControllerInstance) {
        this.loggerInstance = loggerInstance;
        this.config = config;
        this.modelInstance = course;
        this.syllabusModelInstance = courseSyllabus;
        this.courseItemController = courseItemControllerInstance;
    }
    ...
    */
    ...
    async getSelectedCourseNames(req, res) {
        try {
            const {
                courseIds
            } = req.body;
            if (!courseIds || !courseIds.length) {
                return sendClientError({
                    res,
                    responseMessage: 'Valid array of courseIds required'
                });
            }
            const courses = await this.modelInstance.find({
                _id: { $in: courseIds }
            }, {
                name: 1,
                courseType: 1
            ...
            return sendSuccess({
                res,
                responseMessage: 'Course names retrieved successfully',
                responseObject: courses.map(x => ({
                    _id: x._id,
                    name: x.name,
                    courseType: x.courseType.name,
                    courseTypeId: x.courseType.id
                }))
            });
        } catch (error) {
            return sendServerError({
                res,
                responseMessage: 'Unable to retrieve course names'
            });
        }
    }
        ...
        return this.modelInstance.aggregate([
            ...
            },
            {
                "$project": {
                    ...
                    "courseItemsDetails": "$courseItemsData"
                }
            },
            {
                "$group": {
    ...
    addTestsToCourses(test) {
        const { courses, aclMetaData } = test;
        const courseIds = courses
        return new Promise((resolve, reject) => {
            this.modelInstance.find({ _id: { "$in": courseIds } }, 'courseItems', async (err, docs) => {
                if(err) {
                    return reject(err);
                } else {
                    await Promise.all(docs.map(async doc => {
                        const orgCourseItem = doc.courseItems.find(item => item.itemType === 'orgTests');
                        if (!orgCourseItem) {
                            let item = 'orgTests';
                            await this.courseItemController.createItemWithTest({itemName: item, itemType: item,
                                courseId: doc._id, clientId: aclMetaData.clientId,
                                userId: aclMetaData.userId, aclMetaData: aclMetaData, test });
                        } else {
                            await this.courseItemController.addOrgTestToItem({
                                courseItemId: orgCourseItem.id,
                                test
                            });
                        }
                        oldTestSummaryController.createOrgTestSummary(test._id, doc._id);
                    }));
                    return resolve();
                }
            });
    ...
    async getDistinctCourseIds(req, res) {
        const CourseIds = this.fetchDistinctCourseIds();
        CourseIds.then((ids) => {
            return res.json(new ResponseController(200, "Distinct Course Ids", ids));
        });

    }

    async fetchDistinctCourseIds() {
        const distinctCourses = await this.modelInstance.distinct("_id").exec();
        return distinctCourses;
    }

}

export function getCourseControllerInstance() {
    CourseControllerInstance = CourseControllerInstance || new CourseController(loggerInstance, config, getcourseItemControllerInstance());
