...
export class QuestionController {
    ...
    createContentObj(passageContent, locale){
        return new Promise((resolve,reject)=>{
            let contentObj = {
                passageQuestion: passageContent,
                locale: locale,
    ...
    questionLinkedEdit(req, res){
        ...
        .then((responseObject)=>{
            //and delete the ones in request
            ...
            let numberQuestion = notDeletedPreviousQuestions.length + questionTypeArray.length - deleteArray.length;
            let questionArray = [];
            let promiseArray = [];
            let responseObject = {
                passageId: passageId,
            ...
            previousQuestionArray.map((previousQuestion)=>{
                 ...
                 */
                if (languageFlag === 0) {
                    this.createContentObj(passageContent, locale)
                    .then((contentObj)=>{
                        previousQuestion.content.push(contentObj);
                    });
                }
                else {
                    previousQuestion.content[j].passageContent = passageContent;
                }
...
//create contentarray from updated questionarray and the assign it to new questions
            questionTypeArray.map((qType, i) => {
                ...
                question.passageInfo.thisQuestionNumber = notDeletedPreviousQuestions.length + i + 1;//check it
                question.questionType = qType;
                this.createContentObj(passageContent, locale)
                    .then((contentObj) => {
                        promiseArray.push(new Promise((resolve, reject) => {
                            question.content.push(contentObj);
                            question.content.concat(previousContent);
                            ((that) => {
                                question.save()
                                    .then(function (question) {
                                        that.loggerInstance.info("Question saved successfully");
...
//update old questions 
            Promise.all(promiseArray)
                .then(() => {
                    ...
                    return res.json(new ResponseController(200, "Linked Questions Edited", responseObject));
                })
                .catch((err) => {
