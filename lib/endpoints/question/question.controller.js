...
export class QuestionController {
    ...
    publishQuestion(questionId) {
        ...
        this.questionModelInstance.findById(questionId)
        .then(question => {
            if (!question) {
                this.loggerInstance.debug("Question not found");
                return res.json(new ResponseController(404, "Not found question with given ID"));
            ...
            .then((data) => {
                ...
                this.loggerInstance.info("Question Validity Success");
                return res.json(new ResponseController(200, "Question Validity Success", data));
            })
            .catch((err) => {
                this.loggerInstance.error(`Cant be Published as Question is not Valid ${err}`);
                return res.json(new ResponseController(500, "Cant be Published as Question is not Valid", err));
            });
        })
        .catch((err) => {
            this.loggerInstance.error(`DB error getting question ${err}`);
            return res.json(new ResponseController(500, "Error getting question", err));
        });
    }

    checkValidity(question) {

        let tagCheck = ["Difficulty Level", "Complexity Level", ];
        let checkArray = ["questionContent", "solutionContent", "questionHints"];
        let status = [];
