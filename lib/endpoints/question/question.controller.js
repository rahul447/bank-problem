...
export class QuestionController {
    ...
    publishQuestion(req, res, questionId) {
        return new Promise((resolve, reject) => {
            ((that) => {
                that.questionModelInstance.findById(questionId, function (err, doc) {
                    if(err) {
                        ...
                        if(doc.publishId) {
                            that.deleteQuestion(doc.publishId)
                            .then(() => {
                                that.loggerInstance.info("Question Soft Delete Success");
                                ...
                                that.loggerInstance.error(`Soft delete Question Error ${err}`);
                                return res.json(new ResponseController(500, `Soft delete Questio Error`,
                                    err));
                            })
                        } else {
                            doc.status = "PUBLISHED";
