...
export class QuestionController {
    ...
    questionSave(req, res) {
        ...
        req.body.content && req.body.content.map((c) => c.correctAnswer && c.correctAnswer.data && c.correctAnswer.data.map(a => a.value = a.value.replace(/<p>/g, '').replace(/<\/p>/g, '').replace(/  +/g, '')));
    ...
    createQuestionClone(oldObj, bodyObj) {
        return new Promise((resolve, reject) => {
            ...
            tailoredObj.content && tailoredObj.content.map((c) => c.correctAnswer && c.correctAnswer.data && c.correctAnswer.data.map(a => a.value = a.value.replace(/<p>/g, '').replace(/<\/p>/g, '').replace(/  +/g, '')));
    ...
    publishQuestion(questionId, lastUpdateBy) {
        return new Promise((resolve, reject) => {
            ((that) => {
                ...
                exec(function (err, doc) {
                    if(err) {
                        ...
                        if(doc.draftId) {
                            ...
                            that.deleteQuestion(doc.draftId._id, lastUpdateBy)
                            .then(() => {
    ...
    deleteQuestion(questionId, lastUpdateBy) {
        return new Promise((resolve, reject) => {
            ((that) => {
