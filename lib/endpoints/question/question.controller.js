...
export class QuestionController {
    ...
    questionSave(req, res) {
        ...
        req.body.content && req.body.content.map((c) => c.correctAnswer && c.correctAnswer.data && c.correctAnswer.data.map(a => {
    ...
    createQuestionClone(oldObj, bodyObj) {
        return new Promise((resolve, reject) => {
            ...
            tailoredObj.content && tailoredObj.content.map((c) => c.correctAnswer && c.correctAnswer.data && c.correctAnswer.data.map(a => {
                a.value = a.value.replace(/<p>/g, '').replace(/<\/p>/g, '').replace(/  +/g, '');
                return a;
            }));
