...
QuestionSchema.methods.getMaxContentId = () => {
    return new Promise((resolve, reject) => {
        mongoose.models['Question'].find({}, {contentId: 1}).sort({contentId:-1}).limit(1)
        .exec((err, doc) => {
...
QuestionSchema.methods.checkValidity = function() {
    ...
    let status = {'tolerance': true, 'conceptId': true, 'questionContent': true, 'solutionContent': true, 'correctAnswer': true, 'optionsContent': true, 'matrixOptionContent': true, 'difficultyLevel': true, 'difficultyType': true};
    let question = this;

    const schemaFields = Array.from(Object.keys(QuestionSchema.paths)
        .reduce((acc, curr) => {
    ...
    function validate(key) {
        ...
        if(['difficultyLevel', 'difficultyType'].includes(key) &&
            (!question[key] || question[key].length === 0)) status[key] = false;
    ...
    if(this.status === "PUBLISHED" && await this.findStatusById(this._id, this.status) !== this.status) {
        ...
        .then((status) => {
            if(status.length === 0) {
                ...
                next(new Error(`question has values for (${status}) missing `));
            }
        }).catch(err => {
