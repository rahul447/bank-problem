...
import questionsBk from "../question/question-bk.model";
import questionsDirty from "../question/question-dirty.model";
...
export class MigrateController {
    ...
    constructor(loggerInstance, config) {
        ...
        this.questionsBk = questionsBk;
        this.questionsDirty = questionsDirty;
    ...
    async changes(req, res) {
        try {
            ...
            //req.params.id === '26' && await this.correctEmptyCorrectAnswers();
            req.params.id === '27' && await this.contentIdFixer();
            req.params.id === '28' && await this.removeEnglishFromCourseItem();
            req.params.id === '29' && await this.correctTestMode();
            req.params.id === '30' && await this.correctOldContentId();
            req.params.id === '31' && await this.moveQuestionsFromDirtyToMain();
            req.params.id === '32' && await this.cleanMatrixQues();
            ...
            if (question.level.length !== 0) {
                difficultyLevelArray.map((dLevel)=>{
                    if(question.level.toLowerCase().includes(dLevel) &&
                        !question.difficultyLevel.includes(dLevel)){
                        update = true;
                ...
                difficultyTypeArray.map((dType)=>{
                    if(question.level.toLowerCase().includes(dType) &&
                        !question.difficultyType.includes(dType)){
                        update = true;
                ...
                if(update) {
                    ((that) => {
                        that.questions.findOneAndUpdate({ _id: question._id }, question,
                            function (err, data) {
                            if (err) {
                                reject(err)
                            }
            ...
            return new Promise(async function(resolve,reject){
                ...
                for(let count=0;count<=counter;count++){
                    ...
                    for(let question of questionList){
                        await that.updateQuestionObject(question)
                    }
                }
                resolve();
            })
                ...
                await Promise.all(allContents.map(async content => {
                    ...
                    while (skips <= length) {
                        ...
                        await Promise.all(allData.map(async doc => {
                            ...
                            const subjects = doc.subjects ? await this.subjects.find({
                            ...
                            subjects ? doc.aclMetaData.subjects = subjects.map(s => s.name.toLowerCase())
                                : "";
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                */
                ...
                client = client || new this.client({
                    _id: this.config['fiitjee-enterprise-id'],
                    name: 'fiitjee',
                ...
                */
                ...
                const viewAndValidateAccesses = [
                    "VIEW",
                    "VALIDATE"
                ];
                const featureLevelPermissions = [
                    {
                        "featureType": "CONTENT",
                    ...
                    {
                        "featureType": "LANGUAGE",
                        "accessAllowed": viewAndValidateAccesses
                    },
                    {
                        "featureType": "TAG",
                        "accessAllowed": viewAndValidateAccesses
    ...
    /*async correctEmptyCorrectAnswers() {
        this.questions.find({ "content.correctAnswer.data.0": { "$exists": false } }, 'content',
            (err, docs) => {
                // console.log("docs : ", JSON.stringify(docs));
                docs.map(doc => {
                    this.QuestionSafe.find({_id: mongoose.Types.ObjectId(doc._id)}, 'content',
                        (err, safeDocs) => {
                            // console.log("safeDocs : ", JSON.stringify(safeDocs));
                            if(safeDocs[0] && safeDocs[0].content[0] && safeDocs[0].content[0].correctAnswer && safeDocs[0].content[0].correctAnswer.data) {
                                doc.content[0].correctAnswer.data =
                                    safeDocs[0].content[0].correctAnswer.data;
    ...
    async contentIdFixer() {

        let collObj = {"Q": this.questions, "T": this.tests, "A": this.audios, "F": this.formula,"S": this.studyMaterial, "V": this.videos};
        /*let collObj = {"Q": this.questions};*/

        return new Promise(async (resolve) => {
            await Promise.all(Object.keys(collObj).map(async coll => {
                const length = await collObj[coll].count();
                console.log(`We have a total of ${length} count for ${coll}`);
                const limit = 2000;
                let skips = 0;
                let i = 1;
                while (skips <= length) {
                    const allData = await collObj[coll].find({ "publishId": {"$exists": false}})
                        .sort({"_id": 1}).skip(skips).limit(limit);
                    await Promise.all(allData.map(async doc => {
                        if(doc) {
                            //!doc.oldContentId ? doc.oldContentId = doc.contentId: '';
                            doc.oldContentId = doc.contentId;
                            doc.contentId = `${coll}-${i}`;
                            doc.docCounter = i;
                            i++;
                            doc.draftId && await this.updateDraftVersion(collObj[coll], doc.draftId,
                                doc.contentId, doc.docCounter);
                            await doc.save();
                        }
                    }));
                    skips += limit;
                    console.log(`************ ${skips} documents of ${coll} have been processed`);
                }
            }));
            return resolve();
        });
    ...
    updateDraftVersion(coll, draftId, contentId, docCounter) {
        return new Promise((resolve) => {
            coll.findOne({ _id: mongoose.Types.ObjectId(draftId)}, async (err, doc) => {
                if(doc) {
                    //!doc.oldContentId ? doc.oldContentId = doc.contentId: '';
                    doc.oldContentId = doc.contentId;
                    doc.contentId = `${contentId}-DRAFT`;
                    doc.docCounter = docCounter;
                    await doc.save();
                }
                resolve();
            });
    ...
    async removeEnglishFromCourseItem() {
        let courseItem = await this.courseItem.find({"_id":
                    mongoose.Types.ObjectId("5ace21819fe7b01d5807565d")}, {"details.tests": 1});

        courseItem[0].details.tests = courseItem[0].details.tests.filter(tests =>
            !tests.subjects.includes("English"));

        await courseItem[0].save((err) => {
            err ? console.log("error") : console.log("done");
        });
    }

    async correctTestMode() {
        const allBuggyTests = await this.tests.find({ "status": "PUBLISHED", "testMode": { "$ne": 4}},
            {"testMode": 1, "draftId": 1});

        allBuggyTests.map(async test => {
            test.testMode = 4;
            await test.save();
            if(test.draftId) {
                this.findDraftedTest(test.draftId);
            }

        });
    }

    async findDraftedTest(testId) {
        const draftTestData = await this.tests.find({ "_id": testId}, {"testMode": 1});
        draftTestData.map(async draftDoc => {
            draftDoc.testMode = 4;
            await draftDoc.save();
        });
    }

    async correctOldContentId() {
        const length = await this.questionsBk.count();
        const limit = 2000;
        let skips = 0;
        while(skips <= length ) {
            let quesBk = await this.questionsBk.find({}, {contentId: 1, _id: 1}).skip(skips)
                .limit(limit).exec();
            let promises = [];
            quesBk.map(async bkQues => {
                promises.push(new Promise(async (resolve) => {
                    this.questions.findOne({ _id: bkQues._id }, { contentId: 1, oldContentId: 1 },
                        async (err, currQues) => {
                            if(currQues && currQues._id) {
                                currQues.oldContentId = bkQues.contentId;
                                //console.log("currQues.oldContentId : ", currQues.oldContentId);
                                await currQues.save();
                            }
                            resolve();
                        });
    ...
    async moveQuestionsFromDirtyToMain() {
        let dirtyQues = await this.questionsDirty.find({"contentId": { "$in": ["Q-41464","Q-41411","Q-41369","Q-41461","Q-41405","Q-41408","Q-41417","Q-41370","Q-41455","Q-41456","Q-41457","Q-41458","Q-41459","Q-41460","Q-41462","Q-41463","Q-41465","Q-41466","Q-41372","Q-41373","Q-41374","Q-41375","Q-41376","Q-41377","Q-41378","Q-41379","Q-41380","Q-41382","Q-41383","Q-41384","Q-41385","Q-41386","Q-41387","Q-41388","Q-41389","Q-41390","Q-41391","Q-41392","Q-41393","Q-41394","Q-41395","Q-41396","Q-41397","Q-41398","Q-41399","Q-41400","Q-41401","Q-41402","Q-41403","Q-41404","Q-41406","Q-41407","Q-41409","Q-41412","Q-41415","Q-41420","Q-41422","Q-41381","Q-41410","Q-41414","Q-41431","Q-41432","Q-41433","Q-41435","Q-41437","Q-41438","Q-41440","Q-41443","Q-41448","Q-41449","Q-41467","Q-41469","Q-41470","Q-41472","Q-41474","Q-41481","Q-41483","Q-41487","Q-41489","Q-41490","Q-41493","Q-41495","Q-41452","Q-41468","Q-41471","Q-41480","Q-41482","Q-41484","Q-41485","Q-41488","Q-41491","Q-41492","Q-41418","Q-41423","Q-41425","Q-41427","Q-41436","Q-41439","Q-41441","Q-41476","Q-41486","Q-41494","Q-41496","Q-41413","Q-41416","Q-41419","Q-41421","Q-41424","Q-41426","Q-41428","Q-41429","Q-41430","Q-41434","Q-41442","Q-41444","Q-41445","Q-41446","Q-41447","Q-41450","Q-41451","Q-41453","Q-41454","Q-41473","Q-41475","Q-41477","Q-41479"]}}).lean().exec();

        let promises = [];
        dirtyQues.map(async eachQ => {
            promises.push(new Promise(async resolve => {
                Reflect.deleteProperty(eachQ, "__v");
                let eachQuesDoc = new this.questions(Object.assign({}, eachQ));
                eachQuesDoc.save((err, newDoc) => {
                    console.log("err : ", err);
                    resolve();
                });
            }))
    ...
    async cleanMatrixQues() {
        let matrixQuestions = await this.questions.find({"questionType" : "Matrix"}, {content: 1}).exec(); 

        let promises = [];
        matrixQuestions.map(ques => {
            promises.push(new Promise(async (resolve) => {
                ques.content.map(con => {
                    con.correctAnswer.data.map(d => {
                        d.value = d.value.reduce((acc, val) => {
                            acc.indexOf(val) < 0 ? acc.push(val) : '';
                            return acc;
                        },[]);
                    })
                });
