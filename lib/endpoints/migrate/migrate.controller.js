...
export class MigrateController {
    ...
    async moveOldConceptsToNewTree() {
        ...
        let skips = 0;
        while (skips <= length) {
            const questions = await this.questions.find().skip(skips).limit(limit);
    ...
    createTestInfo(){
        ((that)=>{
            that.tests.find({'data.noOfSections':{'$ne':null}}).populate('data.sections.subSection.questions.qId',"conceptId").limit(1).lean().exec(function(err,testList){
                for(let test of testList){
                    ...
                    conceptIdSet.map((conceptId)=>{
                        ...
                        //insert questionId based on conceptId Array.prototype.includes().
