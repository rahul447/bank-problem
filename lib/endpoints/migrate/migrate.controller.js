...
export class MigrateController {
    ...
    mcqOperation() {
        this.questions.find({ questionCode: { $in: [ 0, 8 ] } } , { questionType: 1, questionCode: 1}, {limit:2}, (err, docs) => {
           docs.map(doc => {
    ...
    getQuestionList(){
        return new Promise((resolve,reject)=>{
            ((that)=>{
                that.questions.find({}).lean().exec(function(err,questionList){
                    if(err){
                        reject(err)
                    }
                    else{
    ...
    async updateLevel(){
        return new Promise((resolve,reject)=>{
            ...
            for(let question of questionList){
                console.log(question._id);
                let farziQuestionUpdateStatus = await updateQuestionObject(question)
            }
