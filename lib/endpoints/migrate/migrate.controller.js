...
export class MigrateController {
    ...
    async audioContentIdUpdate() {
        ...
        this.audios.find({ contentId: { "$exists": false } }, (err, docs) => {
            docs.map(doc => {
                doc.contentId = !maxContentId ? 'A1': maxContentId.slice(0, 1) + (parseInt(maxContentId.slice(1)) + 1);
    ...
    async formulaContentIdUpdate() {
        ...
        this.formula.find({ contentId: { "$exists": false }, publishId: { "$exists": false } }, (err, docs) => {
            docs.map(doc => {
                doc.contentId = !maxContentId ? 'F1': maxContentId.slice(0, 1) + (parseInt(maxContentId.slice(1)) + 1);
                maxContentId = doc.contentId;
                doc.save(err => {
                    if(err) {
                        this.loggerInstance.error(`Error in content id updation ${err}`);
    ...
    async questionContentIdUpdate() {
        ...
        this.questions.find({ contentId: { "$exists": false }, publishId: { "$exists": false } }, (err, docs) => {
            docs.map(doc => {
                doc.contentId = !maxContentId ? 'Q1': maxContentId.slice(0, 1) + (parseInt(maxContentId.slice(1)) + 1);
                maxContentId = doc.contentId;
                doc.save(err => {
                    if(err) {
                        this.loggerInstance.error(`Error in content id updation ${err}`);
    ...
    async materialContentIdUpdate() {
        ...
        this.studyMaterial.find({ contentId: { "$exists": false }, publishId: { "$exists": false } }, (err, docs) => {
            docs.map(doc => {
                doc.contentId = !maxContentId ? 'M1': maxContentId.slice(0, 1) + (parseInt(maxContentId.slice(1)) + 1);
                maxContentId = doc.contentId;
                doc.save(err => {
                    if(err) {
                        this.loggerInstance.error(`Error in content id updation ${err}`);
    ...
    async testContentIdUpdate() {
        ...
        this.tests.find({ contentId: { "$exists": false }, publishId: { "$exists": false } }, (err, docs) => {
            docs.map(doc => {
                doc.contentId = !maxContentId ? 'T1': maxContentId.slice(0, 1) + (parseInt(maxContentId.slice(1)) + 1);
                maxContentId = doc.contentId;
                //console.log("doc : ", doc);
                doc.save(err => {
                    if(err) {
    ...
    async videoContentIdUpdate() {
        ...
        this.videos.find({ contentId: { "$exists": false }, publishId: { "$exists": false } }, (err, docs) => {
            docs.map(doc => {
                doc.contentId = !maxContentId ? 'V1': maxContentId.slice(0, 1) + (parseInt(maxContentId.slice(1)) + 1);
                maxContentId = doc.contentId;
                doc.save(err => {
                    if(err) {
                        this.loggerInstance.error(`Error in content id updation ${err}`);
