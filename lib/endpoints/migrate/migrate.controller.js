...
export class MigrateController {
    ...
    async changes(req, res) {
        ...
        req.params.id === '9' ? await this.insertClass(parseInt(req.query.skip), parseInt(req.query.limit)) : '';
    ...
    insertClass(skip, limit) {
        return new Promise(async (resolve, reject) => {
          let CourseCommaCourseItemAndTestData = await this.getCourseCommaCourseItemAndTest(skip, limit);
          //console.log("CourseCommaCourseItemAndTestData : ", CourseCommaCourseItemAndTestData);
            async.forEach(CourseCommaCourseItemAndTestData, async (doc) => {
                let farziconceptidData = await this.farzitestSummaryCall(doc);

                //console.log("farziconceptidData : ", farziconceptidData);
                ...
                if(farziconceptidData.length > 0) {
                    ...
                    if(classData.length > 0) {
                        ...
                        //console.log("highestClass : ", highestClass);
                        if(highestClass) {
                            await this.updateHighestClass(highestClass, doc);
                        }
    ...
    updateHighestClass(highestClass, doc) {
        return new Promise((resolve, reject) => {
            this.courseItem.findOne({
                "_id": mongoose.Types.ObjectId(doc.courseItemId),
                "details.tests": { $elemMatch: { "paperDetail.id": mongoose.Types.ObjectId(doc.testId) } }
            }, (err, doc) => {
                if(doc && doc.details && doc.details.tests) {
                    doc.details.tests.map(test => {
                        test.class = highestClass;
                    });

                    doc = JSON.parse(JSON.stringify(doc));

                    doc.details.tests.map(test => {
                        test.courseConcepts.map(c => {
                            c.id = mongoose.Types.ObjectId(c.id)
                        });
                        test.paperDetail.id = mongoose.Types.ObjectId(test.paperDetail.id);
                    });

                    this.courseItem.findOneAndUpdate({_id:doc._id}, doc,function (err) {
                        if(err) {
                            console.log("err : ", err);
                            reject(err);
                        }
    ...
    getCourseCommaCourseItemAndTest(skip, limit) {
        return new Promise((resolve, reject) => {
