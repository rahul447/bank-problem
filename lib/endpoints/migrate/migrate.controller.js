...
export class MigrateController {
    ...
    async contentIdFixer() {
        ...
        Object.keys(collObj).map(async coll => this.fixedContentIds(collObj[coll], coll));
    }

    fixedContentIds(coll, start) {
        coll.find({}, {"contentId": 1},(err, content) => {
            let i = 1;
            content.map(con => {
                /*coll.findOneAndUpdate({_id: mongoose.Types.ObjectId(con._id)},
                    { $set: { oldContentId: con.contentId, contentId: `${start}-${i}` }},
                    function (err, doc) {});*/
