...
export class MigrateController {
    ...
    async changes(req, res) {
        await this.courseChanges();
        await this.courseItemChanges();
        await this.packageChanges();
        await this.testChanges();
        await this.questionChanges();
        await this.rulesChanges();
                ...
                docs.map(doc => {
                    promises.push(new Promise((resolve) => {
                        ...
                        doc.save(function(err) {
                            if(err)
                                console.log(" err : ", err);
                            console.log("courseItem Updated");
                            resolve();
                        });
                    }));
                });
            ...
            this.tests.find({}, function (err, docs) {
                ...
                docs.map(doc => {
                    promises.push(new Promise((resolve) => {
                        ...
                        doc.status = doc.status === null ? 'DRAFT' : doc.status;
                        doc.userId = !doc.userId ? null : doc.userId;
