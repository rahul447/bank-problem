...
export class MigrateController {
        ...
        this.questions.find({}, 'content', (err, docs) => {
           docs.map(doc => {
               doc.content.length > 0 && doc.content.map(con => {
                   ...
                   con.correctAnswer.data.length !== corrAns.length && console.log("id : ", doc._id);
               });
           });
        });
    }
    ...
    async contentIdFixer() {
        ...
        //let collObj = {"S": this.studyMaterial};

        return new Promise((resolve, reject) => {
            Object.keys(collObj).map(async coll => {
                return new Promise(async (resolve) => {
                    let inc = { i:1};
                    ["T", "Q", "S"].includes(coll) ?
                        await this.batchProcessingForContentIDFixer(collObj[coll], coll, inc) :
                        await this.fixedContentIds(collObj[coll], coll, inc);
                    console.log(`done for ${coll}`);
                    resolve();
                });
            });
            resolve();
    ...
    fixedContentIds(coll, start, inc) {
        return new Promise((resolve) => {
            coll.find({"$or": [{"publishId": {"$exists": false}},{"publishId": null}]})
            .sort({"_id": 1})
            .exec(async (error, content) => {
                await this.contentIdCleaningOp(content, start, coll, inc)
                resolve();
            });
        });
    }

    async contentIdCleaningOp(content, start, coll, inc) {
        return new Promise((resolve) => {

            content.map(con => {
                return new Promise(async (resolve) => {
                    console.log("i : ", inc.i);
                    con.oldContentId = con.contentId;
                    con.contentId = `${start}-${inc.i}`;
                    con.docCounter = inc.i;
                    inc.i++;

                    con.draftId && this.updateDraftVersion(coll, con.draftId, con.contentId,
                        con.docCounter);
                    con.save((err) => {
                        console.log(`err contentIdCleaningOp: ${err}`);
                        resolve();
                    });
                });
            });
    ...
    updateDraftVersion(coll, draftId, contentId, docCounter) {
        ...
        coll.findOne({ _id: mongoose.Types.ObjectId(draftId)}, (err, doc) => {
            ...
            doc.save((err) => {
                if(err) {
                    console.log("err updateDraftVersion : ", err);
                }
            });
        });
    ...
    async batchProcessingForContentIDFixer(coll, start, inc) {
        return new Promise(async (resolve, reject) => {
            const length = await coll.count();
            const limit = 1000;
            let skips = 0;

            while(skips <= length) {
                const collData = await coll.find(
                    {"$or": [{"publishId": {"$exists": false}}]})
                    .skip(skips).limit(limit).sort({"_id": 1});
                await this.contentIdCleaningOp(collData, start, coll, inc);
                console.log(`************ ${skips} ${start} have been processed`);
                skips += limit;
