...
export class MigrateController {
    ...
    async contentIdFixer() {
        ...
        return new Promise((resolve, reject) => {
            Object.keys(collObj).map(async coll => {
                return new Promise(async (resolve) => {
                    ["T", "Q", "S"].includes(coll) ?
                        await this.batchProcessingForContentIDFixer(collObj[coll], coll) :
                        await this.fixedContentIds(collObj[coll], coll);
    ...
    fixedContentIds(coll, start) {
        return new Promise((resolve) => {
            coll.find({"$or": [{"publishId": {"$exists": false}},{"publishId": null}]})
            .sort({"_id": 1})
            .exec(async (error, content) => {
                await this.contentIdCleaningOp(content, start, coll)
                resolve();
            });
        });
    }

    async contentIdCleaningOp(content, start, coll) {
        return new Promise((resolve) => {
            let i = 1;
            content.map(con => {
                console.log("con : ", con._id);
                return new Promise(async (resolve) => {
                    con.oldContentId = con.contentId;
                    con.contentId = `${start}-${i}`;
                    con.docCounter = i;

                    con.draftId && await this.updateDraftVersion(coll, con.draftId, con.contentId,
                        con.docCounter);
                    i++;
                    con.save((err) => {
                        console.log(`err : ${err}`);
                        console.log(`done for ${start} ${con._id}`);
                        resolve();
                    });
                });
            });
    ...
    async batchProcessingForContentIDFixer(coll, start) {
        return new Promise(async (resolve, reject) => {
            ...
            while(skips <= length) {
                const collData = await coll.find(
                    {"$or": [{"publishId": {"$exists": false}},{"publishId": null}]})
                    .skip(skips).limit(limit).sort({"_id": 1});
                await this.contentIdCleaningOp(collData, start, coll);
