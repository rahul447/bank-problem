...
class PackageController {
    ...
    async getPackageByCourseItem(courseItemId) {
        let packageIds = await this.packageModelInstance.find({"courseDetails.includedItems.itemId":
            courseItemId}, {_id: 1}).exec();
        return packageIds;
    }

}

export function getPackageControllerInstance() {
    PackageControllerInstance = PackageControllerInstance || new PackageController(loggerInstance, config);
    return PackageControllerInstance;
}