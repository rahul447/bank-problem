...
class ContentController {
    ...
    contentList(req, res){
        ...
        let promises = contentTypes.map(this.findContentData.bind(this, req.body.query, conceptIds, questionTypes, status, tagValues, updatedAt, req.body.body.dbQuery));
    ...
    findContentData(reqQuery, conceptIds, questionTypes, status, tagValues, updatedAt, dbQuery, contentType) {
        return new Promise((resolve, reject) => {
            ...
            if (contentType === 'testpapers') {
                ...
                dbQuery.$and.push(...query.$and);
                this.fileModelInstance.paginate(dbQuery, paginationParams).then(content => {
                    resolve(content);
                }, err => {
                    reject(err);
                });
    ...
    findContentDataCount(reqQuery, conceptIds, questionTypes, status,tagValues, dbQuery, contentType) {
        return new Promise((resolve, reject) => {
            let draftIdFlag = 0;
            if(status.length>0 && status.includes("DUAL")){
                draftIdFlag = 1;
    ...
    totalCountByContent(req, res) {
        ...
        let promises = contentTypes.map(this.findContentDataCount.bind(this, req.body.query, conceptIds, questionTypes, status, tagValues, req.body.body.dbQuery));
    ...
    async publishContent(req, res) {
        ...
        switch (contentType) {
            case 'question':
                try {
                    await this.questionInstance.publishQuestion(contentId, aclMetaData);
                ...
                break;
            case 'studymaterial':
                this.studyMaterialInstance.publishMaterial(contentId, aclMetaData)
                .then((data) => {
                ...
                break;
            case 'testpapers':
                this.testInstance.publishTest(contentId, aclMetaData)
                .then((data) => {
                ...
                break;
            case 'formulas':
                this.formulaInstance.publishFormula(contentId, aclMetaData)
                .then((data) => {
                ...
                break;
            case 'videos':
                this.videoInstance.publishVideo(contentId, aclMetaData)
                .then((data) => {
                ...
                break;
            case 'audios':
                this.audioInstance.publishAudio(contentId, aclMetaData)
                .then((data) => {
    ...
    async validateContent(req, res) {
        ...
        switch (contentType) {
            case 'question':
                try {
                    const {validStatus, status}= await this.questionInstance.validateQuestion(contentId);
                    const code = validStatus ? 200 : 500;
                    this.loggerInstance.info(`validateQuestion Done`);
                    return res.json(new ResponseController(code, "validateQuestion Done ", status));
                } catch(err) {
                    this.loggerInstance.error(`validateQuestion Error ${err}`);
                    return res.json(new ResponseController(500, "validateQuestion Error ", err.message));
                }
                break;
            case 'studymaterial':
                try {
                    const {validStatus, status} = await this.studyMaterialInstance.validateMaterial(contentId);
                    const code = validStatus ? 200 : 500;
                    this.loggerInstance.info(`publish Study Material Done`);
                    return res.json(new ResponseController(code, "validate Study Material Done ", status));
                } catch(err) {
                    this.loggerInstance.error(`validate Study Material Error ${err}`);
                    return res.json(new ResponseController(500, "validate Study Material Error ",
                        err.message));
                }
                break;
            case 'testpapers':
                try {
                    const {validStatus, status} = await this.testInstance.validateTest(contentId);
                    const code = validStatus ? 200 : 500;
                    this.loggerInstance.info(`validate Test Done`);
                    return res.json(new ResponseController(code, "validate Test Done ", status));
                } catch(err) {
                    this.loggerInstance.error(`validate Test Error ${err}`);
                    return res.json(new ResponseController(500, "validate Test Error ", err.message));
                }
                break;
            case 'formulas':
                try {
                    const {validStatus, status} = await this.formulaInstance.validateFormula(contentId);
                    const code = validStatus ? 200 : 500;
                    this.loggerInstance.info(`validate Formula Done`);
                    return res.json(new ResponseController(code, "validate Formula Done ", status));
                } catch(err) {
                    this.loggerInstance.error(`validate Formula Error ${err}`);
                    return res.json(new ResponseController(500, "validate Formula Error ", err.message));
                }
                break;
            case 'videos':
                try {
                    const {validStatus, status} = await this.videoInstance.validateVideo(contentId);
                    const code = validStatus ? 200 : 500;
                    this.loggerInstance.info(`validate Video Done`);
                    return res.json(new ResponseController(code, "validate Video Done ", status));
                } catch(err) {
                    this.loggerInstance.error(`validate Video Error ${err}`);
                    return res.json(new ResponseController(500, "validate Video Error ", err.message));
                }
                break;
            case 'audios':
                try {
                    const {validStatus, status} = await this.audioInstance.validateAudio(contentId);
                    const code = validStatus ? 200 : 500;
                    this.loggerInstance.info(`validate Audio Done`);
                    return res.json(new ResponseController(code, "validate Audio Done ", status));
                } catch(err) {
                    this.loggerInstance.error(`validate Audio Error ${err}`);
                    return res.json(new ResponseController(500, "validate Audio Error ", err.message));
                }
                break;
            default:            // for audios
        }
