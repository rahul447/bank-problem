...
export class ContentController {
    ...
    async validateContent(req, res) {
        ...
        switch (contentType) {
            case 'question':
                try {
                    const {validate} = await this.questionInstance.validateQuestion(contentId);
                    this.loggerInstance.info(`validateQuestion Done`);
                    return res.json(new ResponseController(200, "validateQuestion Done ", {validate}));
                } catch(err) {
            ...
            case 'studymaterial':
                try {
                    const {validate} = await this.studyMaterialInstance.validateMaterial(contentId);
                    this.loggerInstance.info(`publish Study Material Done`);
                    return res.json(new ResponseController(200, "validate Study Material Done ", {validate}));
                } catch(err) {
            ...
            case 'testpapers':
                try {
                    const {validate} = await this.testInstance.validateTest(contentId);
                    this.loggerInstance.info(`validate Test Done`);
                    return res.json(new ResponseController(200, "validate Test Done ", {validate}));
                } catch(err) {
            ...
            case 'formulas':
                try {
                    const {validate} = await this.formulaInstance.validateFormula(contentId);
                    this.loggerInstance.info(`validate Formula Done`);
                    return res.json(new ResponseController(200, "validate Formula Done ", {validate}));
                } catch(err) {
            ...
            case 'videos':
                try {
                    const {validate} = await this.videoInstance.validateVideo(contentId);
                    this.loggerInstance.info(`validate Video Done`);
                    return res.json(new ResponseController(200, "validate Video Done ", {validate}));
                } catch(err) {
                    this.loggerInstance.error(`validate Formula Error ${err}`);
                    return res.json(new ResponseController(500, "validate Formula Error ", err.message));
                }
            ...
            case 'audios':
                try {
                    const {validate} = await this.audioInstance.validateAudio(contentId);
                    this.loggerInstance.info(`validate Audio Done`);
                    return res.json(new ResponseController(200, "validate Audio Done ", {validate}));
                } catch(err) {
