...
export class ContentController {
    ...
    constructor(loggerInstance, config, questionInstance) {
    ...
    deleteObject(req, res) {
        ...
        let successResponse = `${req.query.key} Object deleted`;
        let errorResponse = `Error ${req.query.key} deleting object`;
        if (key === "question") {
            ((that) => {
                that.questionModelInstance.update(query,update,options)
                    .then(function () {
                        return res.json(new ResponseController(200, successResponse));
                    })
                    .catch(function (err) {
