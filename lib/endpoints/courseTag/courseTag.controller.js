...
let CourseTagControllerInstance,
...
class CourseTagController {
    ...
    delete(req, res) {
        ...
        this.modelInstance.findOne({ id: mongoose.Types.ObjectId(id) }, (err, doc) => {
            ...
            doc.status = 'DELETED';
            const {updatedBy} = req.aclMetaData;
...
export function getCourseTagControllerInstance() {
    CourseTagControllerInstance = CourseTagControllerInstance || new CourseTagController(loggerInstance, config);
    return CourseTagControllerInstance;
}