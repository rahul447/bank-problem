...
export class ContentTagController {
    ...
    delete(req, res) {
        let id = req.params.id;
        this.modelInstance.findOne({ _id: mongoose.Types.ObjectId(id) }, function (err, doc){
            if (err) {
                this.loggerInstance.debug("Content tag not found");
    ...
    static mapValues(tagArr) {
        ...
        let promises = [];
        /**
         * to remove value from values array if it is blank or undefined or null
         */
        tagArr = tagArr.map(tag=> {
            tag.values = tag.values.filter(value=> value!==undefined && value!=='' && value!==null);
            return tag;
        });
    ...
    getTagTypes(req, res) {
        this.modelInstance.distinct("content.type", { }, (err, result) => {
            if (err) {
                this.loggerInstance.error(`error getTagTypes ${err}`);
                return res.json({"data": err, "status": 500});
            }
            this.loggerInstance.error(`success getTagTypes`);
            return res.json({"data": result, "status": 200});
        });
    }

    getTagDetailsByType(req, res) {

        this.modelInstance.find({"content.type": req.params.type}, (err, docs) => {
            if (err) {
                this.loggerInstance.error(`error getTagDetailsByType ${err}`);
                return res.json({"data": err, "status": 500});
            }
            this.loggerInstance.error(`success getTagDetailsByType`);
            return res.json({"data": docs, "status": 200});
        });
    }
}

export function getContentTagControllerInstance() {
    ContentTagControllerInstance = ContentTagControllerInstance || new ContentTagController({loggerInstance, config});
    return ContentTagControllerInstance;
}