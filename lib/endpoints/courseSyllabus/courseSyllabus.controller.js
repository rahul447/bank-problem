...
export class courseSyllabusController {
    ...
    getAllFormula(data) {
        return Promise.all(data.map(this.getOneFormula))
    }

    getAllVideo(data) {
        return Promise.all(data.map(this.getOneVideo))
    }

    getAllQuestion(data) {
        return Promise.all(data.map(this.getOneQuestion))
    }

    getAllStudyMaterial(data) {
        return Promise.all(data.map(this.getOneStudyMaterial))
    }
            ...
            .then(data => {
                ...
                if (content) {
                    for (let i = 0; i < content.length; i++) {
                        ...
                        else if (content[i].contentType === 'QUESTION') {
                            questionContent.push(content[i].contentId);
                        }
                ...
                return Promise.all([
                    ...
                    this.getAllQuestion(questionContent),
