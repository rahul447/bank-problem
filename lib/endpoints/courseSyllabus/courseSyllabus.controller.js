...
export class courseSyllabusController {
    ...
    getAllPartially(req, res) {
        ...
        this.getCourseSyllabus(querySubject)
            .then(data => {
                responseObject.subject = data[0];
                let queryChapter = {
                    query: { courseId: req.params.courseId, type: 'chapter', ancestors: mongoose.Types.ObjectId(responseObject.subject._id) },
                    sort: { name: 1 }
                };
                return this.getCourseSyllabus(queryChapter)
            })
            .then(data => {
                responseObject.chapter = data[0];
                let queryConcept = {
                    query: { courseId: req.params.courseId, type: 'concept', ancestors: { $all: [mongoose.Types.ObjectId(responseObject.chapter._id), mongoose.Types.ObjectId(responseObject.subject._id)] } },
                    sort: { name: 1 }
                };
