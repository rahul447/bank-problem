...
TestsSchema.methods.checkValidity = function() {
    ...
    let status = {'name': true, 'displayName': true, 'testCode': true, 'testType': true, 'duration': true, 'language': true, 'Rules': true, 'DuplicateSection': true, 'QuestionNotEqual': true, 'DuplicateQuestion': true, 'questionsPublishIssue': true};
    ...
    if(this.status === "PUBLISHED" && await this.findStatusById(this._id, this.status) !== this.status) {
        ...
        .then((status) => {
            if(status.length === 0) {
                ...
                next(new Error(`published because test has values for (${status}) missing `));
            }
        }).catch(err => {
