...
TestsSchema.methods.getMaxContentId = () => {
    return new Promise((resolve, reject) => {
        mongoose.models['Tests'].find({}, {contentId: 1}).sort({contentId:-1}).limit(1)
        .exec((err, doc) => {
...
TestsSchema.methods.checkValidity = function() {
    ...
    let status = {'name': true, 'displayName': true, 'testType': true, 'duration': true, 'language': true, 'Rules': true, 'DuplicateSection': true, 'QuestionNotEqual': true, 'DuplicateQuestion': true, 'questionsPublishIssue': true};
