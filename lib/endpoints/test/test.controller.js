...
class TestController {
            ...
            workSheet.rows.map(async (row) => {
                promises.push(new Promise(async (resolve, reject) => {
                    ...
                    let courseItemData = await this.CourseItemModel.find({ "_id": courseItemId }).lean()
                        .exec();
                    console.log("courseItemData : ", courseItemData)
                    const isNewSchedule = ['true', 'TRUE', 'True', true, "1", 1, "yes", "Yes", "YES"].includes(row[6])
                    console.log("isNewSchedule : ", isNewSchedule)
                    let schArr = courseItemData[0].details.schedule;
                    console.log("schArr : ", schArr)
                    let {alters, scheduleID} = isNewSchedule ? this.newScheduleOp(schArr, row, testData) : this.updateScheduleOp(schArr, row, testData) 

                    await this.CourseItemModel.update({ _id: courseItemId }, {
                        $set: {
                            "details.schedule": schArr
                        }
                    }, { upsert: true }).exec();
                    ...
                    console.log("elasticUpdations : ", elasticUpdations)
    ...
    newScheduleOp(schArr, row, testData) {
        ...
        schArr.push(alters)
        return {alters, scheduleID: alters.scheduleID};
    }

    updateScheduleOp(schArr, row, testData) {
        ...
        schArr.map(sch => String(sch.scheduleID) ===
                        String(selectedSchedule.scheduleID) ? selectedSchedule : sch);
        return {alters, scheduleID: selectedSchedule.scheduleID};
    }
