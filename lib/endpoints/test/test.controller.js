    ...
    createTestClone(oldObj, bodyObj) {
        return new Promise((resolve, reject) => {
            ...
            ((that) => {
                test.save()
                .then(function(test) {
                    that.loggerInstance.info(`Test Clone saved successfully ${question._id}`);
                    resolve(test);
                })
                .catch(function(err) {
            ...
            ((that) => {
                that.testModelInstance.findById(testId, function (err, doc) {
                    if(err) {
                        ...
                        if(doc.publishId) {
                            that.deleteTest(doc.publishId)
                            .then(() => {
                                that.loggerInstance.info("Test Soft Delete Success");
                                ...
                                that.loggerInstance.error(`Soft delete Test Error ${err}`);
                                reject(err);
                            })
                        } else {
                            doc.status = "PUBLISHED";
