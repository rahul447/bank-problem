        ...
        return new Promise(async (resolve, reject) => {
            ...
            workSheet.rows.map(async row => {
                promises.push(new Promise(async (resolve, reject) => {
                    ...
                    let masterConceptCodeForConcept = await this.getMasterConceptForConcept(CourseSyllabus[0].masterIDs)
                    
                    let subjectSearchSet = CourseSyllabus[0].ancestors.reduce((acc, curr) => {
                        acc.add(curr); return acc; }, new Set());

                    let masterConceptCodeForChapter = await this.getMasterConceptCodeForChapter(subjectSearchSet)
                    ...
                    let validChapters = await this.CourseSyllabusModel.find({"_id": { "$in":
                                Array.from(subjectSearchSet)}, "type": "chapter"}, {"name": 1})
                                .lean().exec();
                    ...
                    courseConcepts = courseConcepts.reduce((acc, curr) => {
                        curr.conceptCode = masterConceptCodeForConcept
                        acc.push(curr)
                        return acc
                    }, [])

                    validChapters = validChapters.reduce((acc, curr) => {
                        curr = Object.assign({}, curr, {"conceptCode": masterConceptCodeForChapter})    
                        acc.push(curr)
                        return acc
                    }, [])
                    ...
                    let elasticUpdations = {codeName: testData.name, oldTestId: testData._id,
                        ...
                        "conceptCode" : row[1]};
                    console.log("elasticUpdations : ", JSON.stringify(elasticUpdations));
