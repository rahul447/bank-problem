...
class TestController {
                ...
                exec(async function (err, doc) {
                    if(err) {
                        ...
                        if(doc.draftId) {
                            ...
                            if(invalid.length !== 0) {
                                console.log("test invalid 2: ", invalid);
                            ...
                            if(invalid.length !== 0) {
                                console.log("test invalid 1: ", invalid);
                                reject(new Error(JSON.stringify({invalid, contentId: doc.contentId})));
                            } else if(publish) {
    ...
    scheduleTestHandler(workSheet) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            let promises = [];
            workSheet.rows.map(async (row, index) => {
...
*/
                    ...
                    console.log("packageSet : ", packageSet);

                    let elasticUpdations = {testId: testData._id, testType: testData.settings.testType,
            ...
            workSheet.rows.map(async row => {
                promises.push(new Promise(async (resolve, reject) => {
                    ...
                    let validChapters = await this.CourseSyllabusModel.find({"_id": { "$in":
                                Array.from(subjectSearchSet)}, "type": "chapters"}, {"name": 1}).exec();
    ...
    async updateScheduleTestInElasticSearch(packageIds, courseId, testId, scheduleId, updations) {
        ...
        return new Promise((resolve, reject) => {
            //resolve();
            let theScript = {
                "inline": `ctx._source.testId = ${updations.testId}; ctx._source.name = ${updations
                    .codeName}; ctx._source.mainTestType = ${updations.testType}; ctx._source
                    .mainTestType = ${updations.testType}; ctx._source.syllabus = ${updations.syllabus}; 
                    ctx._source.codeName = ${updations.codeName}; ctx._source.displayName = ${updations
                    .displayName}; ctx._source.duration = ${updations.duration}; ctx._source
                    .availability = ${updations.status};`
            };
    ...
    async addConceptTestInElasticSearch(packageIds, courseId, testId, updations) {
        ...
        packageIds.map(packageId => {
            bulkData.push({ index: { _index: 'all_tests', _type: 'mytype',  } },
                {
                    "testId" : testId,
                    "name" : updations.codeName,
                    "packageId" : packageId,
                    "goalId" : courseId,
                    "testType" : "conceptTest",
                    "codeName" : updations.codeName,
                    "oldTestId" : updations.oldTestId,
                    "displayName" : updations.displayName,
                    "duration" : updations.duration,
                    "availability" : "Live",
                    "subjects" : updations.subjects,
                    "concepts" : updations.concepts,
                    "chapters" : updations.chapters,
                    "conceptCode" : updations.conceptCode
                });
        });
