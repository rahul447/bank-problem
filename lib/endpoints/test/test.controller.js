...
class TestController {
                    ...
                    if(err) {
                        ...
                        if(doc.draftId) {
                            ...
                            that.testModelInstance.findOne({_id: doc._id}, function (err, doc) {
                                that.deleteTest(doc.draftId, aclMetaData)
                                .then(() => {
    ...
    deleteTest(testId, aclMetaData) {
        console.log("testId : ", testId);
        console.log("aclMetaData : ", aclMetaData);
        return new Promise((resolve, reject) => {
            ((that) => {
    ...
    validateOnly(doc) {
        let status = {'name': true, 'displayName': true, 'testType': true, 'duration': true, 'language': true, 'Rules': true, 'DuplicateSection': true, 'DuplicatesubSectionName': true, 'QuestionNotEqual': true, 'DuplicateQuestion': true, 'QuestionCode': true};
                ...
                sec.subSection.map(async subSec => {
                    ...
                    subSec.questions.map(ques => {
                        ...
                        promises.push(new Promise((resolve) =>
                            questionInstance.publishQuestion(ques.qId, test.aclMetaData).then()
                        ));
                    });
                });
            });
