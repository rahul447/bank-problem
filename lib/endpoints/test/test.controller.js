...
class TestController {
    ...
    getTestById(testId, req, res) {
        ...
        .populate('data.sections.subSection.questions.qId', 'questionType passageId content.questionContent contentId')
        .lean()
        .then(test => {
            if (!test) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            for(let i = 2; i <= dataLength - 1; i++) {
                await new Promise(async (resolve, reject) => {
                    ...
                    let testQuery = await this.testModelInstance.find({"contentId": row[0]}, {"name": 1,
                        "displayName": 1, "syllabus": 1, "settings": 1}).exec();

                    let courseQuery = await this.courseModel.find({"name": row[2]}, {"name": 1,
                        "courseItems": 1}).exec();
