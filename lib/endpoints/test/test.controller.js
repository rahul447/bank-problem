...
import { getConceptControllerInstance } from "../concept/concept.controller";
...
class TestController {
    constructor(loggerInstance, config, courseControllerInstance, elasticClient = {}) {
        ...
        this.conceptInstance = getConceptControllerInstance()
                        ...
                        test.data.sections.map(section => {
                            section.subSection.map(subSection => {
                                subSection.questions.map(question => {
                                    ...
                                    question.docCounter = question.qId.docCounter;
                                    question.status = question.qId.status;
    ...
    async addToCourseViaExcel(req, res) {
        this.loggerInstance.debug(`========STARTING TEST EXCEL UPLOAD`);
    ...
    scheduleTestHandler(workSheet) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            firstrow.shift();
            if (!firstrow.every((v, i) => {
                return v.trim() === excelPattern[i].trim()
            })) return reject("COLUMN_PATTERN_ERROR");
            let promises = [];
            workSheet.rows.map(async (row) => {
                promises.push(new Promise(async (resolve, reject) => {
                    row.shift();
                    let { testData, courseData } = await this.getTestAndCourseData(row);
                    /* console.log("testData : ", JSON.stringify(testData)); return false */

                    let testConceptCodes = Array.from(this.getTestConceptCodes(testData));
                    ...
                    let courseItemData = await this.CourseItemModel.find({ "_id": courseItemId }).lean()
                        .exec();
                    const isNewSchedule = !(row[5])
                    let schArr = courseItemData[0].details.schedule;
                    let {alters, scheduleID, schArrChanged} = isNewSchedule ? this.newScheduleOp(schArr, row, testData, testConceptCodes) : this.updateScheduleOp(schArr, row, testData, testConceptCodes) 
                    return false;
                    if(!alters && !scheduleID && !schArrChanged) {
                        reject("CONCEPT_MAPPING_ERROR")
                    } else {
                        await this.CourseItemModel.update({ _id: courseItemId }, {
                        $set: {
                            "details.schedule": schArrChanged
                        }
                    }, { upsert: true }).exec();
                        await this.runTestSummary(testData._id, courseData._id);
                        ...
                        let elasticUpdations = {
                            testId: testData._id, mainTestType: testData.settings.testType,
                            syllabus: testData.syllabus.text, codeName: alters.codeName, displayName: alters.displayName, duration: String(alters.duration), availability: "Live", scheduleDate: alters.scheduleDate,
                        };

                        await this.updateScheduleTestInElasticSearch(Array.from(packageSet), courseData._id, testData._id, scheduleID, elasticUpdations, isNewSchedule);
    ...
    getTestConceptCodes(testData) {
        let testConceptCodeSet = new Set()
        testData.data.sections.map(sec => {
            sec.subSection.map(sub => {
                sub.questions.map(ques => {
                    ques.qId.conceptCode.map(con => {
                        testConceptCodeSet.add(con);
                    });
                });
            });
        });
        return testConceptCodeSet;
    }
        ...
        return new Promise(async (resolve, reject) => {
            ...
            workSheet.rows.map(async row => {
                promises.push(new Promise(async (resolve, reject) => {
                    ...
                    let courseSyllConcepts = Array.from(this.getCourseConcepts(row[2]))

                    console.log("testConceptCodes : ", testConceptCodes)
                    console.log("courseSyllConcepts : ", courseSyllConcepts)
                    if(!(testConceptCodes.length === courseSyllConcepts.length && testConceptCodes.every((value, index) => value === courseSyllConcepts[index]))) return reject("CONCEPT_MAPPING_ERROR")
                    ...
                    let CourseSyllabus = await this.CourseSyllabusModel.find({
                        ...
                        "typeCode": { "$in": row[1].split(",")}
                    }).exec();
                    
                    let subjectSearchSet = CourseSyllabus.reduce((acc, syll) => {
                        syll.ancestors.map(anc => {
                            acc.add(anc)
                        })
                        return acc;
                    }, new Set())
                    
                    let validsubjects = await this.CourseSyllabusModel.find({
                        "_id": {
                            "$in":
                                Array.from(subjectSearchSet)
                        }, "type": "subject"
                    }, { "name": 1 }).exec();

                    let validChapters = await this.CourseSyllabusModel.find({
                        "_id": {
                            "$in":
                                Array.from(subjectSearchSet)
                        }, "type": "chapter"
                    ...
                    let testGroupObj = {
                        ...
                        "status": "Live",
                        "duration": testData.settings.duration || "20",
                    ...
                    let elasticUpdations = {
                        "duration": String(testGroupObj.duration), "availability": "Live", "displayName": row[3],
                        "scheduleId": testData._id,
                        "id": testData._id,
                        "chapters": validChapters.reduce((acc, curr) => { 
                            acc.push(curr.name)
                            return acc
                        }, []),
                        "subjects": validsubjects.reduce((acc, curr) => { 
                            acc.push(curr.name)
                            return acc 
                        }, []),
                        "syllabus": [
                            {
                                "chapterName": validChapters[0].name,
                                "conceptArray": courseConcepts.reduce((acc, curr) => {
                                    acc.push(curr.name)
                                    return acc 
                                }, [])
                            }
                    ...
                    await this.addConceptTestInElasticSearch(Array.from(packageSet), courseData._id, elasticUpdations);
                    console.log("done for row");
                    resolve();
                }));
            });
    ...
    async getTestAndCourseData(row) {
        return new Promise(async (resolve) => {
            ...
            let courseQuery = await this.courseModel.find({ "_id": row[2] }, {
                "name": 1,
                "courseItems": 1
            }).exec();
    ...
    runTestSummary(testId, courseId) {
        ...
        let url;
        if (NODE_ENV === 'staging') {
            url = `${this.config.http.protocol}://${this.config.http.domain}:${this.config.http.port}/createTestSummary`;
        } else {
            url = `http://localhost:3021/createTestSummary`;
        }
    ...
    async updateScheduleTestInElasticSearch(packageIds, courseId, testId, scheduleId, updations, isNewSchedule) {
        return isNewSchedule ? await this.insertScheduleElasticOp(packageIds, updations, courseId, scheduleId, testId) : await this.updateScheduleElasticOp(packageIds, courseId, scheduleId, updations)
    }

    async addConceptTestInElasticSearch(packageIds, courseId, updations) {

        let bulkData = [];
        packageIds.map(packageId => {
            bulkData.push({ index: { _index: 'cms-staging-concept-tests-v2', _type: 'Tests', } }, {...updations, packageId, "goalId": courseId});
        });
        return new Promise((resolve, reject) => {
    ...
    async findTestElasticSearch({ packageIds, courseId, scheduleId }) {
        ...
        const result = await this.elasticClient.search(searchObj);
    ...
    async getMasterConceptForConcept(masterConceptIds) {
        let conceptDoc = await this.conceptInstance.getConceptDocById(masterConceptIds)
        return this.getMaxConceptCode(conceptDoc);
    }

    getMaxConceptCode(conceptDoc) {
        const conceptCodes = conceptDoc.reduce((acc, curr) => {
            acc.push(curr.conceptCode);
            return acc;
        }, []);
        return conceptCodes.sort((a, b) => a - b).pop();
    }

    async getMasterConceptCodeForChapter(ancestorsSet) {
        let conceptMasterIds = await this.CourseSyllabusModel.find({ "ancestors": { "$in": Array.from(ancestorsSet) }, "type": "concept" }, { "masterIDs": 1 }).exec()
        const conceptMasterArr = conceptMasterIds.reduce((acc, curr) => acc.concat(curr.masterIDs), [])
        const conceptDoc = await this.conceptInstance.getConceptDocById(conceptMasterArr)
        return Promise.resolve(this.getMaxConceptCode(conceptDoc));
    }

    newScheduleOp(schArr, row, testData, testConceptCodes) {

        let courseConcepts = Array.from(this.getCourseConcepts(row[2]))
        console.log("testConceptCodes : ", testConceptCodes)
        console.log("courseConcepts : ", courseConcepts)
        return false
        if(!(testConceptCodes.length === courseConcepts.length && testConceptCodes.every((value, index) => value === courseConcepts[index]))) return {} 

        let alters = {
            "testId" : row[1],
            "scheduleID" : String(schArr.reduce((acc, curr) => { 
                acc = Number(curr.scheduleID) > acc ? Number(curr.scheduleID) : acc
                return acc
            }, 0) + 1),
            "scheduleTime" : "",
            "paperDetail": {
        ...
        schArr.push(alters)
        let schArrChanged = [].concat(schArr)
        return {alters, scheduleID: alters.scheduleID, schArrChanged};
    }

    updateScheduleOp(schArr, row, testData, testConceptCodes) {
        ...
        let selectedSchedule = schArr.filter(sch => String(sch.scheduleID) === String(row[5]))[0];
        ...
        let schArrChanged = schArr.map(sch => String(sch.scheduleID) ===
                        String(selectedSchedule.scheduleID) ? selectedSchedule : sch);
        return {alters, scheduleID: selectedSchedule.scheduleID, schArrChanged};
    }

    async insertScheduleElasticOp(packageIds, updations, courseId, scheduleId, testId) {
        let bulkData = [];
        packageIds.map(pkg => {
            bulkData.push({ index: { _index: 'all_tests', _type: 'tests'} }, 
            Object.assign({}, updations, {
                name: updations.displayName, 
                "packageId" : pkg, 
                "goalId" : courseId,
                "testType" : "mainTest",
    ...
    async updateScheduleElasticOp(packageIds, courseId, scheduleId, updations) {
    ...
    async getCourseConcepts(courseId) {
        let courseConceptData = await this.CourseSyllabusModel.find({courseId: courseId, type: "concept"}, {"typeCode": 1, _id: 0}).exec()
        return courseConceptData.reduce((acc, curr) => {
            acc.add(curr.typeCode)
            return acc
        }, new Set())
    }
}
