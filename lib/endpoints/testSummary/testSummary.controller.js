    ...
    async getTestsWithSyllabus (req, res) {
        try {
            ...
            if (!req.body.courseId) {
                this.loggerInstance.debug("CourseID is Mandatory");
                return res.json(new ResponseController(400, "CourseId is mandatory"));
            }
            let query = {};
            ...
            query['subjects.id'] = req.body.subjectId;
            if ($in) {
                query.testId = { $in : $in };
            }
            if ($nin) {
            ...
            let tests = await this.modelInstance.find(query)
                                .populate('testId').skip(skips).limit(pageSize).sort('concepts.0.name');
