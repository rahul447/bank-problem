...
let TestSummaryControllerInstance,
...
class TestSummaryController {
            ...
            .then(testSummary => {
                ...
                testSummary.concepts.map(concept => {
                    concept.questions.map(question => {
                        let newQuestion = questionList.find(item => item.question.id === question._id);
                        let newConcept = {
                            id: concept.id,
                            name: concept.name
                        };
    ...
    async getTestsWithSyllabus (req, res) {
        try {
            ...
            const {
                limit, offset, $in, $nin, q, testType, type, status, dbQuery
            } = req.body;
            const page = parseInt(offset, 10) || 1;
            const pageSize = parseInt(limit, 10) || 10;
            const skips = pageSize * (page - 1);
            query.$match.isOrgTest = type === 'mine' ? true : {$ne: true};
            if (testType) {
                query.$match.testType = testType;
            }
            if (status) {
            ...
            if (type === 'mine') {
                const originalQuery = query.$match;
                if (dbQuery && dbQuery.$and && dbQuery.$and.length) {
                    const condition = dbQuery.$and[0];
                    if (condition && condition.$or) {
                        const permissions = condition.$or;
                        permissions.map(permission => {
                            if (Object.keys(permission).includes('aclMetaData.clientId')) {
                                permission['aclMetaData.clientId'].$in.map(id => mongoose.Types.ObjectId(id));
                            } else if (Object.keys(permission).includes('aclMetaData.createdBy.id')) {
                                permission['aclMetaData.createdBy.id'] = mongoose.Types.ObjectId(permission['aclMetaData.createdBy.id']);
                            } else if (Object.keys(permission).includes('$and')) {
                                permission['$and'].map(sharedPermission => {
                                    if (Object.keys(sharedPermission).includes('aclMetaData.allowedTo.user.id')) {
                                        sharedPermission['aclMetaData.allowedTo.user.id'] = mongoose.Types.ObjectId(sharedPermission['aclMetaData.allowedTo.user.id']);
                                    }
                                });
                            }
                        });
                ...
                query.$match = dbQuery || { $and: [] };
                query.$match.$and.push(originalQuery);
            }
            ...
            let respTests = tests.map(test => {
                let respTest = {};
                test.testId[0] && test.testId[0]._id && test.testId[0]._id ?
                ...
                respTest.subjectIds = test.subjects.map(s => s.id);
                respTest.testCode = test.testCode;
                if (type === 'mine') {
                    respTest.status = test.status;
                }
                return respTest;
            });
    ...
    async getTestConcepts(req, res) {
        try {
            ...
            const subjectId = testSummary.subjects[0].id;
    ...
    async getAllTestConcepts(req, res) {
        try {
            ...
            let testSummaryData = query.length ? await this.modelInstance.find({ $or: query }).populate('testId') : [];
            ...
            this.loggerInstance.error("Error in test summary", error);
    ...
    async getTestChapterName (req, res) {
        try {
            const {
                data
            } = req.body;
            const responseObjectData = await this.modelInstance.find({
                $or: data
            }, {
                'chapters.name': 1,
                'testId': 1,
            ...
            console.log(responseObjectData)
            const responseObject = responseObjectData.filter(
                x => x.chapters.length
            ).map(x => ({
                testId: x.testId,
                courseId: x.courseId,
                chapter: x.chapters[0].name
            }));
            return sendSuccess({
                res,
                responseObject
            ...
            this.loggerInstance.error("Error in test chapter names", error);
            return sendServerError({
                res,
                responseMessage: 'Error in test chapter names'
            });
        }
    }
}

export function getTestSummaryControllerInstance() {
    TestSummaryControllerInstance = TestSummaryControllerInstance || new TestSummaryController(loggerInstance, config);
    return TestSummaryControllerInstance;
}